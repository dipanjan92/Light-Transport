...
def push(state: Dict[str, Any], values: jnp.ndarray) -> Dict[str, Any]:
    # values is a vector of shape (4,) : [start, end, parent_idx, is_second_child]
    ptr = state["stack_ptr"]
    stack = state["stack"]
    # Because JAX arrays are immutable, we “update” by using index update.
    stack = stack.at[ptr].set(values)
    state = {**state, "stack": stack, "stack_ptr": ptr + 1}
    return state


def pop(state: Dict[str, Any]) -> (jnp.ndarray, Dict[str, Any]):
    ptr = state["stack_ptr"] - 1
    state = {**state, "stack_ptr": ptr}
    values = state["stack"][ptr]
    # values: [start, end, parent_idx, is_second_child]
    return values, state
